 ...
 */
public class VistacreditoController implements Initializable {
    ...
    public TableView<Movimiento> Tablamovcredito;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Tablamovcredito.setItems(creditos); // Vincular la lista a la TableView
...
collsado.setCellFactory(new Callback<TableColumn<Movimiento, Double>, TableCell<Movimiento, Double>>() {
  ...
  controlador.agregarTotales();
    ...
    private void buscarMovimientos(String searchText) {
        ...
        Tablamovcredito.setItems(filteredData);
    }
    ...
    private String calcularTotalGastos() {
        ...
        for (Movimiento movimiento : creditos) {
    ...
    public void ajustarAnchoColumnas() {
        ...
        for (TableColumn<Movimiento, ?> column : Tablamovcredito.getColumns()) {
            ...
            if (columnName.equals("Código")) {
                ...
                for (int i = 0; i < Tablamovcredito.getItems().size(); i++) {
    ...
    // Calcular totales
    double totalDebe = creditos.stream().mapToDouble(Movimiento::getDebe).sum();
    double totalHaber = creditos.stream().mapToDouble(Movimiento::getHaber).sum();
    double totalSaldo = creditos.stream().mapToDouble(Movimiento::getSaldo).sum();
