 ...
 */
public class VistatransferenciasController implements Initializable {
    ...
    public TableView<Movimiento> Tablamov;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Tablamov.setItems(transferencias); // Vincular la lista a la TableView
...
collsado.setCellFactory(new Callback<TableColumn<Movimiento, Double>, TableCell<Movimiento, Double>>() {
    ...
    public void refrescar() {
        Tablamov.setItems(transferencias);
        Tablamov.refresh();
    }
...
private String calcularTotalGastos() {
    ...
    for (Movimiento movimiento : transferencias) {
        totalGastos += movimiento.getDebe(); // Suponiendo que 'debe' representa gastos
    }
    DecimalFormat df = new DecimalFormat("#,##0.00");
    return df.format(totalGastos); // Formatear el total a dos decimales
}
...
private String calcularTotalIngresos() {
    ...
    for (Movimiento movimiento : transferencias) {
        System.out.println("haber" + movimiento.getHaber());
        totalIngresos += movimiento.getHaber(); // Suponiendo que 'haber' representa ingresos
    }
    DecimalFormat df = new DecimalFormat("#,##0.00");
    return df.format(totalIngresos); // Formatear el total a dos decimales
}
    ...
    public void ajustarAnchoColumnas() {
        ...
        for (TableColumn<Movimiento, ?> column : Tablamov.getColumns()) {
            ...
            if (columnName.equals("Código")) {
                ...
                for (int i = 0; i < Tablamov.getItems().size(); i++) {
    ...
    // Calcular totales
    double totalDebe = transferencias.stream().mapToDouble(Movimiento::getDebe).sum();
    double totalHaber = transferencias.stream().mapToDouble(Movimiento::getHaber).sum();
    double totalSaldo = transferencias.stream().mapToDouble(Movimiento::getSaldo).sum();
